...
public class LocalBeamSearchCorePool {
	...
	public void generateBeamSearchCores(TimeTable outputTable,StudCollection studCol, LockableResultSet LRS, boolean op) {
		...
		for (int i = 0; i < 4; i++) {
			threads.add(new LocalBeamSearchCore(LRS.getResultTable(i), results, op));
		}
		...
		outputTable.getMyStuds().setAllLists(LRS.getResultStudCol(0).getAllStuds(), LRS.getResultStudCol(0).getThreeCourseStuds(), LRS.getResultStudCol(0)
